...
typedef unsigned int (*readwriteSipDataCallback )(unsigned int*hostsrcP,unsigned short *portsrcP,unsigned int*hostdstP,unsigned short *portdstP ,unsigned char *data,int *lenP);
readwriteSipDataCallback readSipDataCallbackP;
readwriteSipDataCallback writeSipDataCallbackP;
void setReadWriteCallback(readwriteSipDataCallback readSipDataCallbackP,readwriteSipDataCallback writeDataCallP );
void setReadWriteCallback(readwriteSipDataCallback readSipDataP,readwriteSipDataCallback writeDataCallP )
{
	readSipDataCallbackP = readSipDataP;
	writeSipDataCallbackP = writeDataCallP;
				   ...
				   int tolen)
{
    ...
    pj_sockaddr_in *tmpAdd;
    unsigned long outlenULong;
	char *newP;
	...
	tmpAdd = (pj_sockaddr_in *)to;
	portDst = tmpAdd->sin_port;//ntohs(tmpAdd->sin_port);
	
	#ifdef OPENVPN_SUPPORT
	if(writeSipDataCallbackP)
	{
		 pj_sockaddr_in localSock;
		unsigned short portSer = 0;	
		unsigned int tolen = sizeof(pj_sockaddr_in);
		pj_sock_getsockname(sock,(struct sockaddr*)&localSock,&tolen);
		portSer = localSock.sin_port;//ntohs(localSock.sin_port);
		
		writeSipDataCallbackP(&localSock.sin_addr.s_addr,&portSer, &tmpAdd->sin_addr.s_addr,&portDst,(unsigned char*)buf,(int*)len);
		return PJ_SUCCESS;
	}
	
	#endif

	 portDst = ntohs(tmpAdd->sin_port);
	/*if(portDst>4000)
	{	
		printf("\nsendvport = %d ",portDst);
	}*/
	 if(portDst==_ENCRIPT_PORT_)
	{
		if(*len>10)
		{
			...
			//printf("\n enc data");
		newP =(char*) EncryptInfo(MUNDU_DATA_ENUM,(unsigned char*)buf,(unsigned long)*len,&outlenULong,0,_NO_HEX_);
		//	printf("\nsendvport = %d len =%d",portSer,outlenULong);
		
		buf = newP;
		outlenULong = sendto(sock, (const char*)newP, outlenULong, flags, 
				     ...
				     int *fromlen)
{
	...
	if(readSipDataCallbackP)
	{
		...
		*len = recvfrom(sock, (char*)buf, *len, flags, 
		...
		tmpAdd = (pj_sockaddr_in *)from;
		readSipDataCallbackP(&tmpAdd->sin_addr.s_addr,&portdstr,0,0,buf,len);
		tmpAdd->sin_port = portdstr;
		  if (*len < 0) 
			  return -1;
		return PJ_SUCCESS;
	}
    ...
    *len = recvfrom(sock, (char*)buf, *len, flags, 
	...
	return PJ_RETURN_OS_ERROR(pj_get_native_netos_error());
			...
			struct pj_sockaddr_in *tmpAdd;
			int portSer;
			tmpAdd = (pj_sockaddr_in *)from;
			PJ_SOCKADDR_RESET_LEN(from);
			portSer = ntohs(tmpAdd->sin_port);
			/*if(portSer>4000)
			printf("\n				recvport = %d ",portSer);
*/
			if(portSer==_ENCRIPT_PORT_)
			
			{
				unsigned long  decLen=0;
				char *newP=0;
				newP = (char*)DecryptInfo(MUNDU_DATA_ENUM,(unsigned char*)buf,*len,&decLen,_NO_HEX_);
				printf("\n recv = %s\n\n",newP);
				//printf("\nrecvport = %d len =%d",portSer,decLen);
				*len = decLen;
				memmove(buf,newP,*len);
				free(newP);
				newP = buf;
				newP[decLen]=0;
			}
